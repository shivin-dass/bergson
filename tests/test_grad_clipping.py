...
def test_clip_scales_update_by_coef(clip_dataset):
    ...
    def run(max_grad_norm):
        model = _build_model()
        trainer, state = Trainer.initialize(model, torchopt.sgd(lr), max_grad_norm)
        stream = DataStream(clip_dataset, batch_size=len(clip_dataset), device="cpu")
        new = trainer.step(state, stream[0], inplace=False, trace=False)
...
def test_no_clip_is_bit_identical(clip_dataset):
    ...
    def run(max_grad_norm):
        ...
        trainer, state = Trainer.initialize(model, opt, max_grad_norm)
        stream = DataStream(clip_dataset, batch_size=2, device="cpu")
        with tempfile.TemporaryDirectory() as ckpt_dir:
            return trainer.train(state, stream, inplace=True, save_dir=ckpt_dir)
...
def _magic_scores(dataset, max_grad_norm, weights=None):
    ...
    trainer, fwd_state = Trainer.initialize(model, opt, max_grad_norm)
...
def _final_query_loss(dataset, max_grad_norm, weights):
    ...
    stream.weights.data.copy_(weights)
    with tempfile.TemporaryDirectory() as ckpt_dir:
        state = trainer.train(state, stream, inplace=True, save_dir=ckpt_dir)
